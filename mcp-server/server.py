...
async def list_corpora() -> dict:
    ...
    # --- NEW HUMAN-READABLE SUMMARY BLOCK ---
    human_lines = [
...
async def list_objects(
    ...
    for r in records:
        ...
        if quality_tier is not None and r.get("quality_tier") != quality_tier:
            continue
        # Try registered id_field first, then common alternatives
...
async def get_object(
    ...
    records  = await get_cached(corpus)
    id_field = CORPORA[corpus]["id_field"]
    target   = object_id.lower().replace(" ", "")

    for r in records:
        # Try registered id_field first, then common alternatives
        for field in [id_field, "galaxy", "cluster", "cluster_id", "name", "id"]:
            val = r.get(field)
            if val and str(val).lower().replace(" ", "") == target:
                return {"corpus": corpus, "record": r}
...
async def search_metadata(
    ...
    for r in records:
        ...
        if field_val is not None and str(field_val).lower() == target:
            matches.append({
...
async def filter_objects(
    ...
    for r in records:
        ...
        if val is None:
            continue
        # Accept native numerics or string-encoded floats (e.g. "65.0" from CSV)
        if isinstance(val, (int, float)):
            ...
            try:
                ...
                continue  # Non-numeric string field — skip gracefully
...
async def get_corpus_schema(corpus: str) -> dict:
    ...
    first        = records[0]
    # Use columns dict if present; otherwise infer dynamically from first data row
