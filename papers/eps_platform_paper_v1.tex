...
(where applicable), and the $K$-band surface brightness profiles $\Sigma_{\rm disk}$
...
The corpus includes 84 SPARC quality-1 galaxies (the ``golden sample'' used
in the omega correction analysis of Flynn \& Cannaliato 2025), identified by
the \texttt{quality\_tier=1} flag.
...
Of 129 total galaxies, 24 pass the ``omega-ready'' criterion (at least 2
resolved rings with valid $V_{\rm rot}$ and $R$ values), yielding a dwarf
...
\begin{itemize}
\item \textbf{Flagship} ($\sim$72B): Qwen2.5-72B-Instruct fine-tuned on
      all four EPS corpora via Unsloth QLoRA. Target: research-grade
      astrophysics RAG on the EPS dual-node cluster (192 GB VRAM combined).
\item \textbf{Desktop} ($\sim$31B): Gemma 4 31B Dense (TeichAI Claude
      Opus distillation v2) fine-tuned for local inference on RTX 5090
      hardware (32 GB VRAM).
\item \textbf{Laptop} ($\sim$7B): Qwen2.5-7B-Instruct fine-tuned and
      optimized for CPU inference, targeting in-classroom use via
      Google Colab free tier.
\item \textbf{Vision Expert} ($\sim$72B): Qwen2-VL-72B for scientific
      figure reading --- extracting kinematic values from ALMA plots,
      rotation curve figures, and scatter diagrams into structured JSONL.
\end{itemize}

All fine-tuning will use Unsloth QLoRA with bnb-4bit quantization.
Fine-tuned adapter weights will be published to HuggingFace at
\texttt{eps-research/} upon completion.
...
\bibitem[Koribalski et al.(2018)]{koribalski2018}
